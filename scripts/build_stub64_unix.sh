#!/usr/bin/env bash
# build_stub64_unix.sh — Unix/macOS compatible ARM64 blob builder
# Extracts symbol offsets from ELF, prepends 24-byte header (3 x uint64 LE)
...
with open(out_path, 'wb') as f:
    f.write(hdr + raw)
print(f"[+] vm_interp.bin: {len(hdr)+len(raw)} bytes (vm_entry=0x{off1:X} vm_entry_token=0x{off2:X} _token_table_va=0x{off3:X})")
PY
